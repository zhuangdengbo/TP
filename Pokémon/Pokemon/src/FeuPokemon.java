public class FeuPokemon extends Pokemon{
    public FeuPokemon(int numeroNational, String nom, int niveau, int hp,
                      int attaque, int defense, int attaqueSpeciale, int defenseSpeciale, int vitesse) {
        super(numeroNational, nom, "Feu", niveau, hp, attaque, defense, attaqueSpeciale, defenseSpeciale, vitesse);
    }

    @Override
    public void attaquer(Pokemon adversaire) {
        int puissance = 50;  // 假设固定威力
        ...
        System.out.println(getNom() + " 使用火系攻击！");
        if (getType().equals("Feu") && adversaire.getType().equals("Plante"))
            System.out.println("超级有效！");
        else if (getType().equals("Feu") && adversaire.getType().equals("Eau"))
